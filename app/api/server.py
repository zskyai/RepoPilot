...
<body>
  ...
  <script>
    async function runRepoPilot() {
      ...
      document.getElementById('compressed_context').textContent = JSON.stringify(data.analysis?.compressed_context || {}, null, 2);
      document.getElementById('decision_tree').textContent = JSON.stringify(data.analysis?.decision_tree || {}, null, 2);
